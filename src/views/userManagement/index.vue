            ...
            <Row>
              <Col>
                <Button
                  ...
                  v-if="auth.tf_user_edit"
                  icon="plus-round"
                >新增</Button>
                <Button type="error" icon="close" @click="delModal" v-if="auth.tf_user_delete">删除</Button>
              </Col>
              <Col></Col>
            </Row>
          </div>
      ...
      >
        <Row>
          ...
          <Col span="22">
            <FormItem label="手机号" prop="phone">
              <Input
                v-model="editMaterialForm.phone"
                style="width: 100%"
                placeholder="请输入手机号"
                :maxlength="11"
              ></Input>
        ...
        <Row>
          <Col span="22">
            <FormItem label="密码" prop="newPassword1">
              <Input
                v-model="updatePwdForm.newPassword1"
                type="password"
                placeholder="请输入密码"
                :maxlength="50"
                style="width: 100%"
...
export default {
                  ...
                  h(
                    "Button",
                    {
                      props: {
                        type: "primary",
                        size: "small",
                        disabled: !this.auth.tf_user_edit
                      },
                      on: {
                        click: () => {
                  ...
                  h(
                    "Button",
                    {
                      props: {
                        type: "error",
                        size: "small",
                        disabled: !this.auth.tf_user_delete
                      },
                      on: {
                        click: () => {
  ...
  methods: {
    getcreatedTime(createdTime) {
      this.formItem.beginTime = this.$getcreatedTime(
        createdTime,
        this.formItem.beginTime,
        this.formItem.endTime
      ).beginTime;
      this.formItem.endTime = this.$getcreatedTime(
        createdTime,
        this.formItem.beginTime,
        this.formItem.endTime
      ).endTime;
    ...
    updatePwdSubmit() {
      ...
      this.$request.post(
        "/api/user/adminUpdatePassword",
        qs.stringify(updatePwdParams),
        res => {
          this.$refs.table.init();
          if (res.statusCode === 200) {
