<template>
  <div>
    ...
    <Card class="search-card" style="margin-top:10px;">
    ...
    <!-- <Modal class="modal" width="300" v-model="isShowEvaluation" :title="evaluationTitle">
      ...
      <div class="evaluationList">
        <span class="tag" v-for="(item,index) in modalLabel" :key="index">{{item}}</span>
      </div>
      <div class="evaluationcontent">{{evaluationcontent}}</div>
    </Modal> -->
  </div>
</template>
<style scoped>
              ...
              return h(
                "a",
                {
                  ...
                  on: {
                    click: () => {
                      this.$request.post(
                        "/api/emaint/repairProblem/viewByWorkOrderNo",
                        qs.stringify({ workOrderNo: params.row.workOrderNo }),
                        res => {},
                        res => {
                          if (res.statusCode == 200) {
                            sessionStorage.setItem(
                              "paramid",
                              res.responseResult.id
                            );
                            this.$router.push({
                              name: "WorkDetail"
                            });
