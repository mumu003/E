  ...
  <div>
          ...
          <div class="search-row">
            <Row>
              <Col>
                <Button
                  type="primary"
                  @click="addMaterialModal=true"
                  v-if="auth.tf_role_edit"
                  icon="plus-round"
                >新增</Button>
    ...
    >
      <Form
        ...
        :label-width="100"
        :rules="ruleEditMaterial"
      >
        <Row>
          <Col span="22">
            <FormItem label="角色名称" prop="name">
              <Input
                v-model="editMaterialForm.name"
                placeholder="请输入角色名称"
                :maxlength="50"
                style="width: 100%"
              ></Input>
          ...
          <Col span="22">
            <FormItem label="角色说明" prop="explain">
              <Input
                v-model="editMaterialForm.explain"
                type="textarea"
                :rows="4"
                style="width: 100%"
                placeholder="请输入角色说明"
    ...
    <Modal v-model="roleModal" title="配置权限" @on-cancel="resetTree">
      <div>
        <p class="role_p">角色名称：{{roleName}}</p>
        <!-- <p class="role_p">角色说明：{{explain}}</p> -->
      </div>
      <!-- <div  v-if="isLoading">
        <Spin></Spin>
      </div>-->
      <div>
        <Tree :data="tableData" show-checkbox @on-check-change="checkChangeRole"></Tree>
      </div>

      <div slot="footer">
        <Button @click="resetTree">取消</Button>
        <Button :loading="isSubmit" :disabled="isSubmit" type="primary" @click="roleSubmit">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
                    ...
                    "Button",
                    {
                      ...
                      on: {
                        click: () => {
                          ...
                          this.explain = params.row.explain;
  ...
  methods: {
    ...
    checkChangeRole(data) {
      this.selectList = [];
      this.getChecked(this.tableData);
    },
    //重置数据
    resetTree() {
      this.getpermissionList();
    ...
    //回显本角色权限
    showOwnRole(arr, sArr) {
      arr.forEach(item => {
        sArr.forEach(sitem => {
          // if (item.id == sitem && item.children && item.children.length > 0) {
          ...
          if (item.children && item.children.length > 0) {
            this.showOwnRole(item.children, sArr);
          }
        });
      });
    },
    ...
    roleSubmit() {
      ...
      this.isSubmit = true;
      this.$request.post(
        "/api/emaint/role/configuration/permission?permissionId=" +
          idList +
          "&roleId=" +
          this.roleId,
        {},
    ...
    getRolepermission(id) {
      ...
      this.isLoading = true;
      this.$request.post(
        "/api/emaint/role/permissionPage",
        qs.stringify(RoleParam),
        res => {
          if (res.statusCode == 200) {
    ...
    delAll() {
      ...
      this.$request.post(
        "/api/emaint/role/remove",
        qs.stringify({ id: delList }),
        res => {
